...
namespace opengl
{
	...
	**/
	...
	extern "C" int luaopen_love_graphics(lua_State * L)
	{
		...
		unsigned char* buffer = love::filesystem::null::Filesystem::Instance()->getFileData("graphics.lua", "r", &size);
		if (luaL_loadbuffer(L, (const char*)buffer, size, "graphics.lua") == 0)
			lua_call(L, 0, 0);
		delete[] buffer;
#endif
