...
--			print("key", b, u)
			...
			if love.keyboard then love.keyboard.setKeyState(b, true) end
		end,
		keyreleased = function (b, u)
			if love.keyreleased then love.keyreleased(b, u) end
		...
		mousepressed = function (x,y,b)
			if love.mouse then 
				love.mouse.setPosition(x, y) 
				love.mouse.setButtonState(b, true)
			end
			-- todo: for android's no mousemove
		...
		mousereleased = function (x,y,b)
			if love.mouse then 
				love.mouse.setPosition(x, y) 
				love.mouse.setButtonState(b, false)
			end
			-- todo: for android's no mousemove
			if love.update then love.update(dt) end
...
function love.boot()
	...
	--pcall(love.filesystem.setSource, "D:/MyProject/zz_love2d/platform/msvc2008/projectes/demo/")
...
function _love_key(action, k)
	love.event.push(action, k)
end

function _love_mouse(action, x, y, b)
	love.event.push(action, x, y, b)
end
