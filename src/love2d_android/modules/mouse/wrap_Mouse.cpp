...
namespace mouse
{
	...
	int w_setButtonState(lua_State * L)
	{
		Mouse::Button b;
		if (Mouse::getConstant(luaL_checkstring(L, 1), b))
		{
			bool isDown = (bool)luax_toboolean(L, 2);
			instance->setButtonState(b, isDown);
		}
		return 0;
	}
	...
	static const luaL_Reg functions[] = {
		...
		{ "isDown", w_isDown },
		{ "setButtonState", w_setButtonState },
		{ 0, 0 }
	};
	...
	extern "C" int luaopen_love_mouse(lua_State * L)
	{
		if (instance == 0)
		{
			try
			{
				instance = new love::mouse::null::Mouse();
			}
			catch (Exception & e)
			{
